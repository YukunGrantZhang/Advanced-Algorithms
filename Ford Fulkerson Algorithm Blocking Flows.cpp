/*
 * Ford Fulkerson Algorithm Blocking Flows.cpp
 *
	...
	for (i = 0; i < N; i++)
	{
		...
		while (ptr != NULL)
		{
			if (ptr->selected == 1)
			{
				...
				if (ptr->value > 0)
				{
					ptr->value = ptr->value - lowest_value;
				}
			}
...
void DFS(Graph* graph, int vertex)
{
	...
	else
	{
		...
		for (i = 0; i < N; i++)
		{
			...
			while (ptr != NULL)
			{
				if (ptr->dest == vertex && ptr->selected == 1)
				{
					backwards_vertex = i;
					found_vertex = true;
					break;
